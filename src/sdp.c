...
/*
 * Compute unacknowledged frame count from sequence numbers.
 * tx_seq = next SEQ to use; peer_ackseq = last ACKSEQ from peer.
 * SEQ space is 0..14 (15 values; 0xF = sentinel = no frames acked).
 */
static uint8_t seq_unacked(uint8_t tx_seq, uint8_t peer_ackseq)
{
    if (peer_ackseq == SDP_SEQ_NONE)
        return tx_seq; /* all frames since seq 0 are unacked */
    return (uint8_t)((tx_seq + 15u - peer_ackseq - 1u) % 15u);
}
...
/* Write one byte with escaping. Returns 0 on success, -1 on HAL error. */
...
static void process_frame(sdp_ctx_t *ctx)
{
    ...
    if (ctx->rx_pos < 5) return; /* too short */
    ...
    if (ctx->rx_pos != (uint8_t)(len_field + 5u)) {
        ctx->diag.crc_errors++;
        return;
    }
    ...
    if (actual != expected) {
        ctx->diag.crc_errors++;
        /* Send NAK with the raw seq_ack byte */
    ...
    /* Update window: peer has acknowledged up to ackseq */
    ctx->peer_ackseq = ackseq;

    /* Update last-received time and ack state */
    ctx->rx_ackseq   = seq;
    ctx->ack_pending = 1;
    ...
    /* Transition to LINKED on any valid HDLC frame if in SYNC_ACK.
     * Reset all sequence state so the window formula starts clean. */
    if (ctx->state == SDP_STATE_SYNC_ACK) {
        ctx->state       = SDP_STATE_LINKED;
        ctx->tx_seq      = 0;
        ctx->peer_ackseq = SDP_SEQ_NONE;
        /* rx_ackseq was already set to the received frame's seq above */
    }
    ...
    case SDP_T_ACK:
        ctx->last_ack_ms = ctx->hal->millis(ctx->hal->user);
        ctx->ack_pending = 0;
        /* Window already updated via peer_ackseq above */
        return;
    ...
    default:
        /* ANNOUNCE and all application frames delivered to on_frame */
        ctx->on_frame(ctx, &frame, ctx->user);
        return;
    }
}
...
sdp_status_t sdp_send_announce(sdp_ctx_t *ctx)
{
    ...
    st = ctx->on_build_announce(payload, &len, ctx->user);
    if (st != SDP_OK) return st;

    return transmit_frame(ctx, SDP_T_ANNOUNCE, 0, 0, SDP_SEQ_NONE, payload, len);
}
...
/*
 * sdp_connect uses a local on_frame wrapper to intercept the ANNOUNCE frame
 * so it can be returned via out_announce without requiring ctx storage.
 */
typedef struct {
...
static void connect_on_frame(sdp_ctx_t *ctx, const sdp_frame_t *frame, void *user)
{
    ...
    if (frame->type == SDP_T_ANNOUNCE) {
        ...
        cc->got_announce = 1;
        /* Transition to LINKED — reset sequence state fresh */
        ctx->state       = SDP_STATE_LINKED;
        ctx->tx_seq      = 0;
        ctx->peer_ackseq = SDP_SEQ_NONE;
        ctx->rx_ackseq   = SDP_SEQ_NONE;
...
sdp_status_t sdp_connect(sdp_ctx_t *ctx, uint32_t timeout_ms, sdp_frame_t *out_announce)
{
    ...
    /* Temporarily replace on_frame to intercept ANNOUNCE */
...
sdp_status_t sdp_rx_byte(sdp_ctx_t *ctx, uint8_t byte)
{
    ...
    /* Device-mode sync hunt: count consecutive 0xAA bytes.
     * Only active after sdp_listen(); host side bypasses this entirely. */
    if (ctx->sync_hunt_enabled) {
        if (ctx->state == SDP_STATE_SYNC_HUNT) {
            if (byte == SDP_SYNC_BYTE) {
                ...
                /* Non-0xAA, non-FLAG: reset counter, discard byte */
                ctx->sync_count = 0;
                return SDP_OK;
            }
            /* FLAG byte falls through to frame parser */
        } else if (ctx->state == SDP_STATE_SYNC_ACK) {
            /* Discard 0xAA during hold-off; other bytes go to frame parser */
...
sdp_status_t sdp_tick(sdp_ctx_t *ctx)
{
    ...
    /* Hold-off expiry (device mode only) */
    ...
    /* ACK timer: send T_ACK if pending and 50 ms elapsed */
